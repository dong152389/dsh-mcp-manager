...
| Commands | User-selected STDIO MCP executable plus the plugin's Node HTTP Bridge | Executables are resolved and started with argument arrays, not user-provided shell command strings. On Windows, resolved `.cmd` / `.bat` shims are delegated to `cmd.exe` for compatibility while preserving the configured arguments. A user-provided command can still have the operating-system permissions of the DSH process. |
