...
(ctx, harness) => {
  ...
  function disposeActiveConnection(s) {
    ...
    if (s.pendingTransport) { try { s.pendingTransport.close(); } catch {} s.pendingTransport = null; }
  }
  ...
  async function doConnect(s) {
    ...
    const isStale = () => s.connectionId !== connectionId;
    clearReconnectTimer(s);
    disposeActiveConnection(s);
    s.cookies = {};
    s.stale = false;
    ...
    if (s.config.transport === 'openapi') {
      ...
      if (isStale()) { unregisterTools(s); return false; }
    ...
    const transport = (s.config.transport === 'stdio')
      ? createStdioTransport(Object.assign({}, s.config, { name: s.name }))
      : createUnifiedHttpTransport(s.config, s);
    s.pendingTransport = transport;
    const cancelIfStale = (session) => {
      if (!isStale()) return false;
      if (session) { try { session.dispose(); } catch {} }
      try { transport.close(); } catch {}
      if (s.pendingTransport === transport) s.pendingTransport = null;
      return true;
    };
    ...
    if (cancelIfStale(session)) return false;
  ...
  async function connectServer(name) {
    ...
    s.stale = false;
    try {
      const connected = await doConnect(s);
      if (connected === false) return { ok: false, message: '连接已取消' };
      cancelReconnect(s);
      try { await persistServer(s); } catch {}
      return {
        ok: true,
  ...
  harness.handle('mcp_update', async (a) => {
    try {
      return await updateServer(a && typeof a.name === 'string' ? a.name : '', a && a.payload);
    } catch (err) { return { ok: false, message: String((err && err.message) || err) }; }
  });
