...
(ctx, harness) => {
  ...
  // 解析 Set-Cookie 响应头（可能包含多个 cookie，取各段第一个 ';' 前的键值）
  ...
  function createSseTransport(cfg, server) {
    let stream = null;
    let endpoint = null;
    let onMessage = null;
    let onEnd = null;
    ...
    const transport = {
      ...
      async start() {
        ...
        await new Promise((resolve, reject) => {
          const t = ctx.timeout(() => {
            startResolve = null;
            reject(new Error('等待 SSE endpoint 事件超时（15 秒）'));
          }, 15000);
      ...
      async send(msg, opts) {
        ...
        const res = await bridgeRequest('POST', url, headers, JSON.stringify(msg), (opts && opts.timeoutMs) || 60000, opts && opts.signal);
        if (!res.ok) throw new Error('SSE 消息发送失败: ' + res.error);
      ...
      setHandlers(h) { onMessage = h.onMessage; onEnd = h.onEnd; },
      notifyCancelled() {},
      close() { if (stream) { try { stream.cancel(); } catch {} stream = null; } },
    };
    function openStream() {
      let url = cfg.url;
      if (sessionId) url = appendQuery(url, 'sessionId=' + encodeURIComponent(sessionId));
      return bridgeSse(url, Object.assign({}, buildHeaders(cfg, server.cookies), { accept: 'text/event-stream' }), {
        onHeaders: (h) => {
  ...
  function createHttpTransport(cfg, server) {
    let sessionId = null;
    const transport = {
      kind: 'http',
      sessionId,
      ...
      async send(msg, opts) {
        ...
        let res = await bridgeRequest('POST', cfg.url, headers, JSON.stringify(msg), timeoutMs, opts && opts.signal);
        if (!res.ok) throw new Error('HTTP 请求失败: ' + res.error);
        if (cfg.cookieSession && res.headers && res.headers['set-cookie']) applyCookies(server.cookies, res.headers['set-cookie']);
        if (res.headers['mcp-session-id']) { sessionId = res.headers['mcp-session-id']; transport.sessionId = sessionId; }
        if (res.status === 202 && res.headers.location) {
          const loc = resolveUrl(cfg.url, res.headers.location);
          for (let i = 0; i < 20; i++) {
  ...
  function addServer(args) {
    ...
    if (!['stdio', 'sse', 'http', 'openapi'].includes(transport)) {
      return { ok: false, message: 'transport 必须是 stdio / sse / http / openapi' };
    }
    const cfg = {
      transport,
    ...
    if (transport === 'stdio') {
      ...
      if (typeof args.url !== 'string' || !/^https?:\/\//i.test(args.url.trim())) {
        return { ok: false, message: (transport === 'sse' ? 'SSE' : '流式 HTTP') + ' 类型需要填写有效的服务器 URL（http(s)://...）' };
      }
    ...
    servers.set(name, s);
    console.log('[mcp] 已添加服务器', name, '（' + transport + '）');
    return { ok: true, message: '已保存服务器 "' + name + '"（' + transport + '），使用 mcp_connect 连接' };
  }
  ...
  function scheduleSseReconnect(s, err) {
    ...
    if (s.sseAttempts > 3) {
      s.status = 'error';
      s.lastError = 'SSE 流断开且重连失败: ' + String((err && err.message) || err);
      return;
    }
    const delay = 1500 * s.sseAttempts;
    console.log('[mcp:' + s.name + '] SSE 断开，' + delay + 'ms 后重连（第 ' + s.sseAttempts + ' 次）');
    ctx.timeout(() => {
      if (s.status !== 'connected') return;
      s.status = 'connecting';
      s.lastError = null;
      doConnect(s).then(() => {
        ...
        s.status = 'error';
        s.lastError = 'SSE 重连失败: ' + String((e && e.message) || e);
      });
    }, delay);
  }
  ...
  async function doConnect(s) {
    ...
    const transport = (s.config.transport === 'stdio')
      ...
      : (s.config.transport === 'sse' ? createSseTransport(s.config, s) : createHttpTransport(s.config, s));
    await transport.start();
    const session = createSession(s.name, transport, {
      onToolsChanged: () => scheduleToolRefresh(s),
      onStreamEnd: (err) => { if (s.config.transport === 'sse') scheduleSseReconnect(s, err); },
    });
  ...
  async function connectServer(name) {
    ...
    try {
      ...
      return {
        ok: true,
        message: '服务器 ' + name + ' 已连接（' + s.config.transport + '）',
  ...
  defineMgmt('mcp_add', '添加一个 MCP 服务器配置（stdio / sse / http / openapi 四种传输）。添加后用 mcp_connect 连接，连接后服务器工具会自动注册为模型工具。', {
    name: str('服务器唯一名称（用于后续连接/断开/移除），如 my-filesystem'),
    notes: str('服务器注释（选填，说明用途）'),
    transport: { type: 'string', enum: ['stdio', 'sse', 'http', 'openapi'], description: '传输方式：stdio（本地进程）、sse（SSE 传输）、http（Streamable HTTP）、openapi（OpenAPI 规范转工具）' },
    ...
    env: strObj('环境变量（字符串键值，stdio 会传给子进程；sse/http 一并保存）'),
    url: str('[sse/http] 服务器端点 URL（http(s)://...）'),
  ...
  harness.handle('mcp_disconnect', async (a) => disconnectServer(a && typeof a.name === 'string' ? a.name : ''));
  harness.handle('mcp_remove', async (a) => removeServer(a && typeof a.name === 'string' ? a.name : ''));
  harness.handle('mcp_save', async (args) => {
    try { return addServer((args && typeof args === 'object') ? args : {}); }
    catch (err) { return { ok: false, message: String((err && err.message) || err) }; }
  });
  ...
  console.log('[mcp] MCP 管理器已启动（stdio / SSE / Streamable HTTP / OpenAPI）');
};
