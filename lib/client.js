...
.dshmcp-tools-badge-btn:hover {
  background: rgba(16, 185, 129, 0.18);
  border-color: rgba(16, 185, 129, 0.45);
  color: #047857;
  transform: translateY(-1px);
}
.dshmcp-tools-badge-active {
...
.dshmcp-metric-pill-btn {
  cursor: pointer;
  user-select: none;
  font: inherit;
  transition: all 0.15s ease;
}
.dshmcp-metric-pill-btn:hover {
  border-color: var(--dsw-alias-brand-primary, #2563eb);
  background: rgba(37, 99, 235, 0.06);
}

/* === Tools Drawer inside Card === */
.dshmcp-tools-drawer {
  margin-top: 12px;
  padding: 12px 14px;
  border-radius: 9px;
  ...
  flex-direction: column;
  gap: 10px;
  animation: dshmcp-fade-in 0.2s ease;
}
@keyframes dshmcp-fade-in {
...
.dshmcp-tools-search-wrap {
  position: relative;
  display: inline-flex;
  align-items: center;
  width: min(100%, 240px);
}
.dshmcp-tools-search-icon {
  position: absolute;
  left: 8px;
  color: var(--dsw-alias-label-tertiary, #9ca3af);
  pointer-events: none;
...
.dshmcp-tools-search:focus {
  border-color: var(--dsw-alias-brand-primary, #2563eb);
  box-shadow: 0 0 0 2px rgba(37, 99, 235, 0.12);
}
.dshmcp-tools-search-clear {
  position: absolute;
  right: 6px;
  border: none;
  background: transparent;
...
.dshmcp-tool-copy:hover {
  background: var(--dsw-alias-bg-layer-1, #ffffff);
  color: var(--dsw-alias-brand-primary, #2563eb);
  border-color: var(--dsw-alias-brand-primary, #2563eb);
}
.dshmcp-tool-param-count {
  font-size: 11px;
  color: var(--dsw-alias-label-tertiary, #9ca3af);
  background: var(--dsw-alias-bg-layer-2, #f3f4f6);
  padding: 1px 6px;
...
.dshmcp-tool-params-toggle:hover {
  text-decoration: underline;
}
.dshmcp-tool-params-table {
  margin-top: 4px;
  padding: 8px 10px;
  border-radius: 6px;
...
`;
    ...
    const Icons = {
      ...
      ChevronDown: (props) => h("svg", { className: (props && props.className) || "", width: 12, height: 12, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2.2, strokeLinecap: "round", strokeLinejoin: "round" },
        h("polyline", { points: "6 9 12 15 18 9" })
      ),
      Search: () => h("svg", { width: 13, height: 13, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" },
        h("circle", { cx: 11, cy: 11, r: 8 }),
        h("line", { x1: 21, y1: 21, x2: 16.65, y2: 16.65 })
      ),
      Copy: () => h("svg", { width: 12, height: 12, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" },
        h("rect", { x: 9, y: 9, width: 13, height: 13, rx: 2, ry: 2 }),
        h("path", { d: "M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" })
      ),
      Check: () => h("svg", { width: 12, height: 12, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2.2, strokeLinecap: "round", strokeLinejoin: "round" },
    ...
    function ToolItem(props) {
      ...
      const schema = (tool && tool.parameters) || {};
      const propsObj = schema.properties || (schema.type === "object" && schema.properties) || {};
      const propKeys = Object.keys(propsObj);
      const requiredList = Array.isArray(schema.required) ? schema.required : [];

      return h("div", { className: "dshmcp-tool-item" },
        h("div", { className: "dshmcp-tool-top" },
          h("div", { className: "dshmcp-tool-name-wrap" },
            h("span", { className: "dshmcp-tool-name" }, tool.name),
            h("button", {
              type: "button",
              className: "dshmcp-tool-copy",
              title: "复制工具名称",
              onClick: copyName
            },
              h(copied ? Icons.Check : Icons.Copy),
              copied ? "已复制" : "复制"
            )
          ),
          propKeys.length > 0 ? h("span", { className: "dshmcp-tool-param-count" }, propKeys.length + " 个参数") : null
        ),
        tool.description ? h("div", { className: "dshmcp-tool-desc" }, tool.description) : null,
        propKeys.length > 0 ? h("div", null,
          h("button", {
            type: "button",
            className: "dshmcp-tool-params-toggle",
            onClick: () => setShowParams(!showParams)
          },
            showParams ? "收起参数详情" : ("查看参数详情 (" + propKeys.length + ")"),
            h(Icons.ChevronDown, { className: "dshmcp-chevron" + (showParams ? " dshmcp-chevron-up" : "") })
          ),
          showParams ? h("div", { className: "dshmcp-tool-params-table" },
            propKeys.map((k) => {
              const p = propsObj[k] || {};
              const isReq = requiredList.includes(k);
              return h("div", { key: k, className: "dshmcp-param-row" },
                h("span", { className: "dshmcp-param-name" }, k),
                p.type ? h("span", { className: "dshmcp-param-type" }, p.type) : null,
                isReq ? h("span", { className: "dshmcp-param-req" }, "必需") : h("span", { className: "dshmcp-param-opt" }, "可选"),
                p.description ? h("span", { className: "dshmcp-param-desc" }, "— " + p.description) : null
              );
            })
          ) : null
        ) : null
    ...
    function ToolsDrawer(props) {
      ...
      const filtered = q
        ? tools.filter(t => (t.name && t.name.toLowerCase().includes(q)) || (t.description && t.description.toLowerCase().includes(q)))
        : tools;

      return h("div", { className: "dshmcp-tools-drawer" },
        h("div", { className: "dshmcp-tools-header" },
          h("div", { className: "dshmcp-tools-title" },
            h(Icons.Zap),
            "已注册工具 (" + tools.length + ")"
          ),
          tools.length > 3 ? h("div", { className: "dshmcp-tools-search-wrap" },
            h("span", { className: "dshmcp-tools-search-icon" }, h(Icons.Search)),
            h("input", {
              className: "dshmcp-tools-search",
            ...
            filter ? h("button", {
              type: "button",
              className: "dshmcp-tools-search-clear",
              title: "清空",
              onClick: () => setFilter("")
            }, "✕") : null
          ) : null
        ...
        filtered.length === 0
          ? h("div", { className: "dshmcp-tools-empty" }, tools.length === 0 ? "暂无已注册的工具信息" : "没有找到匹配的工具")
          : h("div", { className: "dshmcp-tools-grid" },
              filtered.map(t => h(ToolItem, { key: t.name, tool: t }))
            )
      );
    }
    ...
    function McpPanel(props) {
      ...
      const toggleServerTools = (name) => {
        setExpandedServers((prev) => Object.assign({}, prev, { [name]: !prev[name] }));
      };

      const toggleExpandAll = () => {
        const anyExpanded = servers.some((s) => s.status === "connected" && (s.toolCount > 0 || (s.tools && s.tools.length > 0)) && expandedServers[s.name]);
        const next = {};
        if (!anyExpanded) {
          for (const s of servers) {
            if (s.status === "connected" && (s.toolCount > 0 || (s.tools && s.tools.length > 0))) next[s.name] = true;
          }
        }
      ...
      const totalTools = servers.reduce((acc, s) => acc + (s.toolCount || (s.tools ? s.tools.length : 0)), 0);
      ...
      const rows = servers.map((s) => {
        ...
        const isExpanded = !!expandedServers[s.name];
        const toolsList = (s.tools && Array.isArray(s.tools)) ? s.tools : [];
        const count = s.toolCount || toolsList.length || 0;
        ...
        return h("div", { key: s.name, className: "dshmcp-card" },
          ...
          h("div", { className: "dshmcp-card-meta" },
            h("div", { className: "dshmcp-card-meta-left" },
              isConnected ? h("button", {
                type: "button",
                className: "dshmcp-tools-badge dshmcp-tools-badge-btn" + (isExpanded ? " dshmcp-tools-badge-active" : ""),
                title: isExpanded ? "收起工具列表" : ("点击查看注册的 " + count + " 个工具列表"),
                onClick: () => toggleServerTools(s.name)
              },
                h(Icons.Zap),
                count + " 个工具已注册",
                h(Icons.ChevronDown, { className: "dshmcp-chevron" + (isExpanded ? " dshmcp-chevron-up" : "") })
              ) : null,
          ...
          (isConnected && isExpanded) ? h(ToolsDrawer, { tools: toolsList, serverName: s.name }) : null
        );
      });
      ...
      const anyConnectedExpanded = servers.some((s) => s.status === "connected" && (s.toolCount > 0 || (s.tools && s.tools.length > 0)) && expandedServers[s.name]);
      ...
      return h("div", { className: "dshmcp-panel" },
        ...
        h("div", { className: "dshmcp-summary" },
          ...
          h("button", {
            type: "button",
            className: "dshmcp-metric-pill dshmcp-metric-pill-btn",
            title: anyConnectedExpanded ? "收起所有服务器的工具列表" : "展开所有已连接服务器的工具列表",
            onClick: toggleExpandAll
          },
            "可用工具",
            h("strong", { className: "dshmcp-metric-val" }, String(totalTools)),
            h(Icons.ChevronDown, { className: "dshmcp-chevron" + (anyConnectedExpanded ? " dshmcp-chevron-up" : "") })
          ),
