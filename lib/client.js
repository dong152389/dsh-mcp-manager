...
@media (max-width: 680px) {
  ...
  .dshmcp-kvrow { grid-template-columns: 1fr 1fr; }
  .dshmcp-kvrow .dshmcp-btn { grid-column: 1 / -1; justify-self: start; }
}
`;
    ...
    function KeyValueList(props) {
      ...
      return h("div", { className: "dshmcp-kv" },
        ...
        h("button", {
          type: "button",
          className: "dshmcp-btn dshmcp-add-row dshmcp-btn-sm",
    ...
    function AddServerForm(props) {
      ...
      if (transport === "stdio") {
        ...
        dynamic = h(react.Fragment, null,
          ...
          field("环境变量", false,
            h(KeyValueList, { items: envs, setItems: setEnvs, keyPlaceholder: "变量名", valuePlaceholder: "值", addLabel: "环境变量" })
          )
        );
      } else {
        dynamic = h(react.Fragment, null,
          ...
          field("HTTP 请求头", false,
            h(KeyValueList, { items: headers, setItems: setHeaders, keyPlaceholder: "请求头名称", valuePlaceholder: "请求头值", addLabel: "请求头" })
          ),
          field("会话处理", false,
